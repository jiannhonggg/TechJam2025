...
Overview: A scalable data pipeline for detecting reveiw spam under high data drift conditions. This project solves the Cold Start problem by using an offline RAG-based Teacher to generate synthetic data for a lightweight DistilBERT student model. 

In Location reveiws "spam" Patterns evolve raipidly (Data Drift). Traditional classification systtems fall short on new attack types because they lack labeled training data. 

### 1. The Offline Teacher (This Repository)
* **Role:** High-Precision Data Annotator.
* **Engine:** Retrieval-Augmented Generation (RAG) + Ensemble of 3 LLMs (Llama2, DeepSeek, Gemma).
* **Function:** Retrieves similar historical spam patterns from a Vector Store to accurately label new, unstructured reviews.
* **Latency:** ~4s per sample (Acceptable for offline batch processing).

### 2. The Online Student (Target Production Model)
* **Role:** Real-Time Inference.
* **Engine:** DistilBERT / Logistic Regression.
* **Function:** Trained on the high-quality synthetic labels generated by the Teacher.
* **Latency:** <20ms (Required for live user feed).

This project assess and classifys user reviews. It automatically identifies reviews that violate platform policies (advertisements, irrelevant content, or rants without visit) while also detecting valid reviews. Our system combines Retrieval-Augmented Generation (RAG) with an ensemble of three Large Language Models (LLMs) for high accuracy, explainability, and scalability.
